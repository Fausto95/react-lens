...
import { hexAlpha } from "../view/timelineTheme.js";
...
function visibleWorld(view: CascadeViewport): { x0: number; y0: number; x1: number; y1: number } {
...
function drawNode(
  ...
  // Cause rail: quicker to scan than using the entire node as a loud color block.
  ctx.fillStyle = color;
  ...
  if (view.zoom >= 0.48) {
    ...
    const label = ellipsis(ctx, `${node.name}${suffix}`, rect.width - 22);
    ctx.fillText(label, rect.x + 12, rect.y + 14);
  }
  ...
  if (view.zoom >= 0.7) {
    ...
    const meta = node.kind === "aggregate" ? cause : `${cause} · ${node.selfDuration.toFixed(1)}ms`;
    ctx.fillText(ellipsis(ctx, meta, rect.width - 22), rect.x + 12, rect.y + 27);
  }

  // Self-time meter is independent of node width, so causal geometry never lies about time.
