...
export function hitTestClipRects(point: TimelinePoint, candidates: Iterable<ClipRect>): ClipRect | null {
  ...
  for (const rect of candidates) {
    ...
    const score = (insideVisual ? -1000 : 0) + Math.abs(point.x - cx) + Math.abs(point.y - cy) * 0.25;
    if (score < bestScore) {
      bestScore = score;
      best = rect;
    }
